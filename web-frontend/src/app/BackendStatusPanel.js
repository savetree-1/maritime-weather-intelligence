...
export default function BackendStatusPanel({ status }) {
    ...
    <Card sx={{ mb: 2, bgcolor: "background.paper", borderRadius: 3, boxShadow: 4, transition: '0.2s', '&:hover': { boxShadow: 10, transform: 'scale(1.02)' } }}>
      <CardContent>
        <Typography variant="h6" color="primary" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          Backend Status {isRunning ? <PlayCircleIcon color="success" fontSize="small" /> : status === "ok" ? <CheckCircleIcon color="success" fontSize="small" /> : <ErrorIcon color="error" fontSize="small" />}
        </Typography>
